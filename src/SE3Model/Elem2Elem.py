...
class Decode(nn.Module):
    def __init__(self, size, n_reps, out_channels=4, fp=False):
        ...
        # Up sampling
        self.up1 = ConvBlock(Rs*(m//4)             , Rs*(m//4) + RsH*(m//4), stride=1 , size=size, fpix=fp)
        self.t2 = ConvTBlock(Rs*(m//4) + RsH*(m//4), Rs*(m//4) + RsH*(m//4), size=size, fpix=fp)
        self.up2 = ConvBlock(Rs*(m//4) + RsH*(m//4), Rs*(m//2) + RsH*(m//2), stride=1 , size=size, fpix=fp)
        self.up3 = ConvBlock(Rs*(m//2) + RsH*(m//2), Rs_out                , stride=1 , size=size, fpix=fp)
