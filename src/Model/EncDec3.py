...
class Encode(nn.Module):
    def __init__(self, in_dim, size, mult):
        ...
        # Down sampling
        self.down_1 = ConvBlock(in_dim, m,    size, act)
...
class Decode(nn.Module):
    def __init__(self, out_dim, size, mult):
        ...
        # Up sampling
        self.up_1 = ConvBlock(m//8, m//4,    size, act)
        self.up_2 = ConvBlock(m//4, m//2,    size, act)
        self.up_3 = ConvBlock(m//2, m,       size, act)
        self.up_4 = ConvBlock(m,    out_dim, size, act) 
