...
if __name__=='__main__':
    ...
    params_file_name = str(200) + 'Elem2CharmmParams'
    ...
    volumeE, _ = GetVE.load_batch(path_list, rot=False)
    volumeE = torch.einsum('tixyz->txyzi', volumeE) #permute
    volumeC, _ = GetVC.load_batch(path_list, rot=False)
    #volumeC = torch.einsum('tixyz->txyzi', volumeC) #permute
       
    latent = modelEncode(volumeE)
    output = modelDecode(latent)
    output = torch.einsum('txyzi->tixyz', output) #unpermute
    
    Agroup_names = ["C","CA","CT1","CT2","CT2A","CT3","CPH1","CPH2", "CPT",
                    "CY","CP1","CP2","CP3","CC","CAI","N","NR","NH1", "NH2",
                    "NH3","NC2", "NY","O","OC", "OH1", "S"]
